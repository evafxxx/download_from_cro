...
var mrps = document.getElementsByClassName('mujRozhlasPlayer');
var mrpm = mrp = null;
var dp;
var _playlist;
var dnldmp3files=[];
...
if (mrps.length >0) {
  mrp = mrps[0];
  mrpms = mrp.getElementsByClassName('mrp-holder__media');
  if (mrpms.length >0) {
    mrpm = mrpms[0];
  }
}
...
function sleep_between(ms) {
    if (typeof sleepSetTimeout_ctrl !== 'undefined') {
...
function download_mp3(){
  if (mrps.length && typeof  mrp.attributes['data-player'] !== 'undefined') {
        ...
        try {
          ...
          for (var i=0;i<_playlist.length;i++)  {
              ...
              for (var j=0;j<_audiolinks.length;j++)  {
                  ...
                  if (_audiolinks[j]['variant']=='dash' ) {
                      output_text = output_text + href + '\n';
                  } else if (_audiolinks[j]['variant']=='mp3' ||_audiolinks[j]['variant']=='m4a' ) {
                      var suffix = _audiolinks.length ==1 ? '' :'_'+j;
