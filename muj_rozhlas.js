          ...
          for (var j=0;j<_audiolinks.length;j++)  {
                  ...
                  if (_audiolinks[j]['variant']=='dash' ) {
                     ...
                     var suffix = _audiolinks.length ==1 ? '' :'_'+j;
                      var name = title + suffix + '.' + _audiolinks[j]['variant'];
...
if (pws.length && typeof  pws[0].attributes['data-entry'] !== 'undefined') {
try {
    pw = pws[0];
   ...
   function(err, data) {
       if (err !== null) {
        ...
        title =  data.data.attributes.shortTitle
        _audiolinks = data.data.attributes.audioLinks;           
        if (_audiolinks.length) {
